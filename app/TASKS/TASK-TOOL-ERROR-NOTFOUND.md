# TOOL-ERROR-NOTFOUND — 404/오류 경계 화면

**Seq:** 52 · **Category:** TOOLING · **Priority:** M
**출처:** `TASKS/00_TASK_LIST.md` Seq 52 (Table A/B) — 이 파일이 상세화 대상이며, 두 문서가
어긋나면 `TASKS/00_TASK_LIST.md`를 정본으로 하고 이 파일을 갱신한다.
**상태:** 착수 전(PENDING) — 이 문서는 구현 계약이며 구현 완료를 의미하지 않는다.

## Context

404/오류 경계 화면을(를) 담당하는 TOOLING Task다. `docs/06_SRS_UIUX_REVISED.md`와
...
- Implementation Status: **IMPLEMENT**(`docs/PROJECT_SCOPE.md` 분류를 그대로 승계 — `TASKS/00_TASK_LIST.md` Seq 52 행과 동일)
- 관련 Requirement: REQ-FUNC-078
...
- 생성: `src/app/not-found.tsx`, `src/app/error.tsx`

> 이 Task는 위에 나열된 파일만 생성/수정한다. 범위 밖 파일 수정은 "Forbidden" 절을 따른다.

## Functional AC

- 홈 복귀/재시도 등 복구 행동 최소 1개 포함
...
- [ ] TC-1: 홈 복귀/재시도 등 복구 행동 최소 1개 포함
