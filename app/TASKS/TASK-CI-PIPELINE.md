# CI-PIPELINE — GitHub Actions CI(lint/typecheck/unit/smoke)

**Seq:** 63 · **Category:** CI_RELEASE · **Priority:** M
**출처:** `TASKS/00_TASK_LIST.md` Seq 63 (Table A/B) — 이 파일이 상세화 대상이며, 두 문서가
어긋나면 `TASKS/00_TASK_LIST.md`를 정본으로 하고 이 파일을 갱신한다.
**상태:** 착수 전(PENDING) — 이 문서는 구현 계약이며 구현 완료를 의미하지 않는다.

## Context

GitHub Actions CI(lint/typecheck/unit/smoke)을(를) 담당하는 CI_RELEASE Task다. `docs/06_SRS_UIUX_REVISED.md`와
...
- Implementation Status: **IMPLEMENT**(`docs/PROJECT_SCOPE.md` 분류를 그대로 승계 — `TASKS/00_TASK_LIST.md` Seq 63 행과 동일)
- 관련 Requirement: REQ-NF-031
...
- UNIT-MATE-STATE
- TEST-RLS-BASIC
- TOOL-CONTENT-VALIDATION-SCRIPT
...
- PR마다 TypeScript strict, ESLint, Unit Test, Chromium Smoke 실행, main 병합 전 필수
...
- [ ] TC-1: PR마다 TypeScript strict, ESLint, Unit Test, Chromium Smoke 실행, main 병합 전 필수

## Verify

- GitHub Actions 실행 결과(자체 검증)
