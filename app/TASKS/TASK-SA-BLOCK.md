...
**Seq:** 44 · **Category:** SERVER_ACTION · **Priority:** M
**출처:** `TASKS/00_TASK_LIST.md` Seq 44 (Table A/B) — 이 파일이 상세화 대상이며, 두 문서가
어긋나면 `TASKS/00_TASK_LIST.md`를 정본으로 하고 이 파일을 갱신한다.
**상태:** 착수 전(PENDING) — 이 문서는 구현 계약이며 구현 완료를 의미하지 않는다.

## Context

차단/해제을(를) 담당하는 SERVER_ACTION Task다. `docs/06_SRS_UIUX_REVISED.md`와
...
- Implementation Status: **IMPLEMENT**(`docs/PROJECT_SCOPE.md` 분류를 그대로 승계 — `TASKS/00_TASK_LIST.md` Seq 44 행과 동일)
- 관련 Requirement: REQ-FUNC-040
...
- (blocker, blocked) UNIQUE 제약

## Test Cases

- [ ] TC-1: 차단 시 상호 글/프로필/요청 비노출
- [ ] TC-2(보안/개인정보): (blocker, blocked) UNIQUE 제약
