# CMP-SCR005-GUEST-AUTH — 로그인/가입/재설정 Card + 성인확인

**Seq:** 29 · **Category:** COMPONENT · **Priority:** M
**출처:** `TASKS/00_TASK_LIST.md` Seq 29 (Table A/B) — 이 파일이 상세화 대상이며, 두 문서가
어긋나면 `TASKS/00_TASK_LIST.md`를 정본으로 하고 이 파일을 갱신한다.
**상태:** 착수 전(PENDING) — 이 문서는 구현 계약이며 구현 완료를 의미하지 않는다.

## Context

로그인/가입/재설정 Card + 성인확인을(를) 담당하는 COMPONENT Task다. `docs/06_SRS_UIUX_REVISED.md`와
...
- Implementation Status: **IMPLEMENT**(`docs/PROJECT_SCOPE.md` 분류를 그대로 승계 — `TASKS/00_TASK_LIST.md` Seq 29 행과 동일)
- 관련 Requirement: REQ-FUNC-028, 066
...
- 생성: `src/components/scr005/GuestAuthCards.tsx`

> 이 Task는 위에 나열된 파일만 생성/수정한다. 범위 밖 파일 수정은 "Forbidden" 절을 따른다.

## Functional AC

- 로그인/가입(성인확인 포함)/비밀번호 재설정 3개 Card, 로그인 후 가능한 기능 3단계, 보안 안내 배너

## Visual AC

- Desktop 3열 Card, Mobile 1열

## Security/Privacy AC

- 정확한 생년월일 미저장, 성인 여부+확인 시각만 저장

## Test Cases

- [ ] TC-1: 로그인/가입(성인확인 포함)/비밀번호 재설정 3개 Card, 로그인 후 가능한 기능 3단계, 보안 안내 배너
- [ ] TC-2(보안/개인정보): 정확한 생년월일 미저장, 성인 여부+확인 시각만 저장
