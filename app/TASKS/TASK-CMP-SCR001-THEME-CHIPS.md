# CMP-SCR001-THEME-CHIPS — 여행 동기·테마 Chip 6

**Seq:** 9 · **Category:** COMPONENT · **Priority:** S
**출처:** `TASKS/00_TASK_LIST.md` Seq 9 (Table A/B) — 이 파일이 상세화 대상이며, 두 문서가
어긋나면 `TASKS/00_TASK_LIST.md`를 정본으로 하고 이 파일을 갱신한다.
**상태:** 착수 전(PENDING) — 이 문서는 구현 계약이며 구현 완료를 의미하지 않는다.

## Context

여행 동기·테마 Chip 6을(를) 담당하는 COMPONENT Task다. `docs/06_SRS_UIUX_REVISED.md`와
...
- Implementation Status: **IMPLEMENT**(`docs/PROJECT_SCOPE.md` 분류를 그대로 승계 — `TASKS/00_TASK_LIST.md` Seq 9 행과 동일)
- 관련 Requirement: REQ-FUNC-002, 010
...
- 생성: `src/components/scr001/ThemeChips.tsx`

> 이 Task는 위에 나열된 파일만 생성/수정한다. 범위 밖 파일 수정은 "Forbidden" 절을 따른다.

## Functional AC

- Chip 6개, 클릭 시 URL query 필터 상태 반영

## Visual AC

- Desktop wrap, Mobile 가로 스크롤 1줄
...
- [ ] TC-1: Chip 6개, 클릭 시 URL query 필터 상태 반영
