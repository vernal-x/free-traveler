# CMP-SCR004-MATE-DETAIL — 상세 패널(Split/Drawer)

**Seq:** 26 · **Category:** COMPONENT · **Priority:** M
**출처:** `TASKS/00_TASK_LIST.md` Seq 26 (Table A/B) — 이 파일이 상세화 대상이며, 두 문서가
어긋나면 `TASKS/00_TASK_LIST.md`를 정본으로 하고 이 파일을 갱신한다.
**상태:** 착수 전(PENDING) — 이 문서는 구현 계약이며 구현 완료를 의미하지 않는다.

## Context

상세 패널(Split/Drawer)을(를) 담당하는 COMPONENT Task다. `docs/06_SRS_UIUX_REVISED.md`와
...
- Implementation Status: **IMPLEMENT**(`docs/PROJECT_SCOPE.md` 분류를 그대로 승계 — `TASKS/00_TASK_LIST.md` Seq 26 행과 동일)
...
- `design-reference/D-001/DESIGN.md` 컴포넌트 정의: Drawer·Modal
...
- 생성: `src/components/scr004/MateDetailPanel.tsx`

> 이 Task는 위에 나열된 파일만 생성/수정한다. 범위 밖 파일 수정은 "Forbidden" 절을 따른다.

## Functional AC

- 작성자 소개·조건·설명·상태 배지 표시, 연락처 비노출

## Visual AC

- Desktop 우측 패널(목록 유지), Mobile 하단→전체 Drawer

## Security/Privacy AC

- 비공개 신청 메시지는 작성자·요청자만 열람(RLS)

## Test Cases

- [ ] TC-1: 작성자 소개·조건·설명·상태 배지 표시, 연락처 비노출
- [ ] TC-2(보안/개인정보): 비공개 신청 메시지는 작성자·요청자만 열람(RLS)
