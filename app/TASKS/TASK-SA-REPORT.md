# SA-REPORT — 신고 접수 + 상태 변경

**Seq:** 45 · **Category:** SERVER_ACTION · **Priority:** M
**출처:** `TASKS/00_TASK_LIST.md` Seq 45 (Table A/B) — 이 파일이 상세화 대상이며, 두 문서가
어긋나면 `TASKS/00_TASK_LIST.md`를 정본으로 하고 이 파일을 갱신한다.
**상태:** 착수 전(PENDING) — 이 문서는 구현 계약이며 구현 완료를 의미하지 않는다.

## Context

신고 접수 + 상태 변경을(를) 담당하는 SERVER_ACTION Task다. `docs/06_SRS_UIUX_REVISED.md`와
...
- Implementation Status: **IMPLEMENT(축소)**(`docs/PROJECT_SCOPE.md` 분류를 그대로 승계 — `TASKS/00_TASK_LIST.md` Seq 45 행과 동일)
- 관련 Requirement: REQ-FUNC-039, 041, REQ-NF-019
...
- 생성: `src/lib/actions/report.ts`

> 이 Task는 위에 나열된 파일만 생성/수정한다. 범위 밖 파일 수정은 "Forbidden" 절을 따른다.

## Functional AC

- 사유코드+설명 접수, 접수번호 즉시 반환, 상태(OPEN/RESOLVED/DISMISSED) 변경만 지원(개별 제재 없음)
...
- 신고자/피신고자 상세는 Moderator/Admin만

## Test Cases

- [ ] TC-1: 사유코드+설명 접수, 접수번호 즉시 반환, 상태(OPEN/RESOLVED/DISMISSED) 변경만 지원(개별 제재 없음)
- [ ] TC-2(보안/개인정보): 신고자/피신고자 상세는 Moderator/Admin만
