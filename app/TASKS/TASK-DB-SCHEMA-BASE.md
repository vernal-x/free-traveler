# DB-SCHEMA-BASE — Supabase 스키마 마이그레이션(6테이블)

**Seq:** 37 · **Category:** DB · **Priority:** M
**출처:** `TASKS/00_TASK_LIST.md` Seq 37 (Table A/B) — 이 파일이 상세화 대상이며, 두 문서가
어긋나면 `TASKS/00_TASK_LIST.md`를 정본으로 하고 이 파일을 갱신한다.
**상태:** 착수 전(PENDING) — 이 문서는 구현 계약이며 구현 완료를 의미하지 않는다.

## Context

Supabase 스키마 마이그레이션(6테이블)을(를) 담당하는 DB Task다. `docs/06_SRS_UIUX_REVISED.md`와
...
- Implementation Status: **IMPLEMENT**(`docs/PROJECT_SCOPE.md` 분류를 그대로 승계 — `TASKS/00_TASK_LIST.md` Seq 37 행과 동일)
- 관련 Requirement: REQ-NF-013
...
- 생성: `supabase/migrations/0001_core_schema.sql`

> 이 Task는 위에 나열된 파일만 생성/수정한다. 범위 밖 파일 수정은 "Forbidden" 절을 따른다.

## Functional AC

- **정확히 6개 테이블만**: `user_profile`, `mate_post`, `mate_application`, `user_block`, `report`, `external_link_settings`. 여행지·안전·대표·미디어·감사로그 테이블 생성 금지
...
- 개인정보 최소 수집(생년월일 컬럼 없음, `is_adult`+`adult_verified_at`만)

## Test Cases

- [ ] TC-1: **정확히 6개 테이블만**: `user_profile`, `mate_post`, `mate_application`, `user_block`, `report`, `external_link_settings`. 여행지·안전·대표·미디어·감사로그 테이블 생성 금지
- [ ] TC-2(보안/개인정보): 개인정보 최소 수집(생년월일 컬럼 없음, `is_adult`+`adult_verified_at`만)
- [ ] TC-3: 이 Task가 생성/수정하는 테이블이 허용 목록 ['user_profile', 'mate_post', 'mate_application', 'user_block', 'report', 'external_link_settings'] 안에서만 존재

## Verify

- TEST-RLS-BASIC, CI-PIPELINE
