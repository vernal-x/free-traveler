# E2E-MATE-AUTH — 인증·동행·신고·차단·관리자 Smoke

**Seq:** 62 · **Category:** E2E_TEST · **Priority:** M
**출처:** `TASKS/00_TASK_LIST.md` Seq 62 (Table A/B) — 이 파일이 상세화 대상이며, 두 문서가
어긋나면 `TASKS/00_TASK_LIST.md`를 정본으로 하고 이 파일을 갱신한다.
**상태:** 착수 전(PENDING) — 이 문서는 구현 계약이며 구현 완료를 의미하지 않는다.

## Context

인증·동행·신고·차단·관리자 Smoke을(를) 담당하는 E2E_TEST Task다. `docs/06_SRS_UIUX_REVISED.md`와
...
- Implementation Status: **IMPLEMENT**(`docs/PROJECT_SCOPE.md` 분류를 그대로 승계 — `TASKS/00_TASK_LIST.md` Seq 62 행과 동일)
- 관련 Requirement: REQ-FUNC-027~041, 043~045, 066, 077, 080
- 각 Requirement의 축소/제외 사유·확인 방법 원문은 `docs/PROJECT_SCOPE.md`의 해당 Requirement 행을 정본으로 한다(이 파일에서 재서술하지 않음).

## Requirement Ref

REQ-FUNC-027~041, 043~045, 066, 077, 080
...
- `design-reference/SCREEN_ROUTE_CONTRACT.json` → `screens[]` 중 screen_id="SCR-004", screen_id="SCR-005" 항목의 `section_order`/`min_content_counts`/`states`/`prohibited_features`
...
- 생성: `e2e/mate-auth-smoke.spec.ts`

> 이 Task는 위에 나열된 파일만 생성/수정한다. 범위 밖 파일 수정은 "Forbidden" 절을 따른다.

## Functional AC

- 흐름: 가입→로그인→성인확인→동행작성→참가요청→승인/거절→신고→차단→관리자 신고처리/URL설정
...
- 로그아웃 상태에서 쓰기 API 401 확인

## Test Cases

- [ ] TC-1: 흐름: 가입→로그인→성인확인→동행작성→참가요청→승인/거절→신고→차단→관리자 신고처리/URL설정
- [ ] TC-2(보안/개인정보): 로그아웃 상태에서 쓰기 API 401 확인

## Verify

- `npx playwright test e2e/mate-auth-smoke.spec.ts --project=chromium`
