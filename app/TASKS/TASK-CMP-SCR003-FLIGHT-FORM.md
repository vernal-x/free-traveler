# CMP-SCR003-FLIGHT-FORM — 항공 Form·검증·요약·외부이동

**Seq:** 20 · **Category:** COMPONENT · **Priority:** M
**출처:** `TASKS/00_TASK_LIST.md` Seq 20 (Table A/B) — 이 파일이 상세화 대상이며, 두 문서가
어긋나면 `TASKS/00_TASK_LIST.md`를 정본으로 하고 이 파일을 갱신한다.
**상태:** 착수 전(PENDING) — 이 문서는 구현 계약이며 구현 완료를 의미하지 않는다.

## Context

항공 Form·검증·요약·외부이동을(를) 담당하는 COMPONENT Task다. `docs/06_SRS_UIUX_REVISED.md`와
...
- Implementation Status: **IMPLEMENT(축소 일부)**(`docs/PROJECT_SCOPE.md` 분류를 그대로 승계 — `TASKS/00_TASK_LIST.md` Seq 20 행과 동일)
- 관련 Requirement: REQ-FUNC-011~018, REQ-NF-017
...
- 생성: `src/components/scr003/FlightForm.tsx`

> 이 Task는 위에 나열된 파일만 생성/수정한다. 범위 밖 파일 수정은 "Forbidden" 절을 따른다.

## Functional AC

- 국가→지역 종속, 출발일 과거/귀국일<출발일 차단, 요약+비전달 고지, "항공편 보러 가기"(새 탭, `noopener,noreferrer`), URL 미설정 시 오류+재시도
...
- **서버 API 없음(`api/flights/*` 생성 금지)**, DB·로그·분석 이벤트에 원시 입력값 저장 금지

## Test Cases

- [ ] TC-1: 국가→지역 종속, 출발일 과거/귀국일<출발일 차단, 요약+비전달 고지, "항공편 보러 가기"(새 탭, `noopener,noreferrer`), URL 미설정 시 오류+재시도
- [ ] TC-2(보안/개인정보): **서버 API 없음(`api/flights/*` 생성 금지)**, DB·로그·분석 이벤트에 원시 입력값 저장 금지
