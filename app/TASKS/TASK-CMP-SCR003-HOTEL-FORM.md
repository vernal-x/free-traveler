# CMP-SCR003-HOTEL-FORM — 숙소 Form·검증·요약·외부이동

**Seq:** 21 · **Category:** COMPONENT · **Priority:** M
**출처:** `TASKS/00_TASK_LIST.md` Seq 21 (Table A/B) — 이 파일이 상세화 대상이며, 두 문서가
어긋나면 `TASKS/00_TASK_LIST.md`를 정본으로 하고 이 파일을 갱신한다.
**상태:** 착수 전(PENDING) — 이 문서는 구현 계약이며 구현 완료를 의미하지 않는다.

## Context

숙소 Form·검증·요약·외부이동을(를) 담당하는 COMPONENT Task다. `docs/06_SRS_UIUX_REVISED.md`와
...
- Implementation Status: **IMPLEMENT(축소 일부)**(`docs/PROJECT_SCOPE.md` 분류를 그대로 승계 — `TASKS/00_TASK_LIST.md` Seq 21 행과 동일)
- 관련 Requirement: REQ-FUNC-019~026, REQ-NF-017
...
- 생성: `src/components/scr003/HotelForm.tsx`

> 이 Task는 위에 나열된 파일만 생성/수정한다. 범위 밖 파일 수정은 "Forbidden" 절을 따른다.

## Functional AC

- 체크인 과거/체크아웃≤체크인 차단, 나머지 FlightForm과 동일 패턴
...
- **서버 API 없음(`api/hotels/*` 생성 금지)**, 원시 입력값 미저장

## Test Cases

- [ ] TC-1: 체크인 과거/체크아웃≤체크인 차단, 나머지 FlightForm과 동일 패턴
- [ ] TC-2(보안/개인정보): **서버 API 없음(`api/hotels/*` 생성 금지)**, 원시 입력값 미저장
