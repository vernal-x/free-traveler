# CMP-SCR001-DESTINATION-GRIDS — 국내·해외 여행지 Card Grid

**Seq:** 7 · **Category:** COMPONENT · **Priority:** M
**출처:** `TASKS/00_TASK_LIST.md` Seq 7 (Table A/B) — 이 파일이 상세화 대상이며, 두 문서가
어긋나면 `TASKS/00_TASK_LIST.md`를 정본으로 하고 이 파일을 갱신한다.
**상태:** 착수 전(PENDING) — 이 문서는 구현 계약이며 구현 완료를 의미하지 않는다.

## Context

국내·해외 여행지 Card Grid을(를) 담당하는 COMPONENT Task다. `docs/06_SRS_UIUX_REVISED.md`와
...
- Implementation Status: **IMPLEMENT**(`docs/PROJECT_SCOPE.md` 분류를 그대로 승계 — `TASKS/00_TASK_LIST.md` Seq 7 행과 동일)
- 관련 Requirement: REQ-FUNC-001, 002, 004, 005, 007, 009, 010, 068, REQ-NF-004
- 각 Requirement의 축소/제외 사유·확인 방법 원문은 `docs/PROJECT_SCOPE.md`의 해당 Requirement 행을 정본으로 한다(이 파일에서 재서술하지 않음).

## Requirement Ref

REQ-FUNC-001, 002, 004, 005, 007, 009, 010, 068, REQ-NF-004
...
- `design-reference/D-001/DESIGN.md` 컴포넌트 정의: Destination Card
...
- DATA-DESTINATIONS
- TOOL-DESIGN-TOKENS
- SA-FAVORITES-LOCALSTORAGE

## Expected Files

- 생성: `src/components/scr001/DestinationGrid.tsx`, `DestinationCard.tsx`

> 이 Task는 위에 나열된 파일만 생성/수정한다. 범위 밖 파일 수정은 "Forbidden" 절을 따른다.

## Functional AC

- 국내 6·해외 6 최소 카드 수, 필터 AND 조건, 빈 결과 시 안내+초기화 버튼(Lorem/빈 카드 금지), 즐겨찾기 토글은 `localStorage`에만 저장

## Visual AC

- Desktop 4열/Mobile 1열, 카드 hover 시만 `shadow.card`

## Security/Privacy AC

- 즐겨찾기 데이터는 클라이언트 저장만, 서버 전송 없음

## Test Cases

- [ ] TC-1: 국내 6·해외 6 최소 카드 수, 필터 AND 조건, 빈 결과 시 안내+초기화 버튼(Lorem/빈 카드 금지), 즐겨찾기 토글은 `localStorage`에만 저장
- [ ] TC-2(보안/개인정보): 즐겨찾기 데이터는 클라이언트 저장만, 서버 전송 없음
