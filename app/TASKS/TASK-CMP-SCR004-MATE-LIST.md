# CMP-SCR004-MATE-LIST — 동행글 목록/Empty State

**Seq:** 25 · **Category:** COMPONENT · **Priority:** M
**출처:** `TASKS/00_TASK_LIST.md` Seq 25 (Table A/B) — 이 파일이 상세화 대상이며, 두 문서가
어긋나면 `TASKS/00_TASK_LIST.md`를 정본으로 하고 이 파일을 갱신한다.
**상태:** 착수 전(PENDING) — 이 문서는 구현 계약이며 구현 완료를 의미하지 않는다.

## Context

동행글 목록/Empty State을(를) 담당하는 COMPONENT Task다. `docs/06_SRS_UIUX_REVISED.md`와
...
- Implementation Status: **IMPLEMENT**(`docs/PROJECT_SCOPE.md` 분류를 그대로 승계 — `TASKS/00_TASK_LIST.md` Seq 25 행과 동일)
- 관련 Requirement: REQ-FUNC-033, 037
...
- 생성: `src/components/scr004/MateList.tsx`, `MatePostCard.tsx`

> 이 Task는 위에 나열된 파일만 생성/수정한다. 범위 밖 파일 수정은 "Forbidden" 절을 따른다.

## Functional AC

- 최대 8개 + 페이지네이션, 모집중/CLOSED 배지는 조회 시 종료일 계산(배치 잡 없음), 0건 시 완성형 Empty State(필터 초기화+작성 CTA+이용 방법 3줄)

## Visual AC

- Desktop Grid, Mobile 1열

## Security/Privacy AC

- 카드에 공개 연락처·실명 과다 노출 없음

## Test Cases

- [ ] TC-1: 최대 8개 + 페이지네이션, 모집중/CLOSED 배지는 조회 시 종료일 계산(배치 잡 없음), 0건 시 완성형 Empty State(필터 초기화+작성 CTA+이용 방법 3줄)
- [ ] TC-2(보안/개인정보): 카드에 공개 연락처·실명 과다 노출 없음
