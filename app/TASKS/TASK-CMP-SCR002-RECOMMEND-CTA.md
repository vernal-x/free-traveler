# CMP-SCR002-RECOMMEND-CTA — 추천 여행지 4 + CTA Banner

**Seq:** 18 · **Category:** COMPONENT · **Priority:** S
**출처:** `TASKS/00_TASK_LIST.md` Seq 18 (Table A/B) — 이 파일이 상세화 대상이며, 두 문서가
어긋나면 `TASKS/00_TASK_LIST.md`를 정본으로 하고 이 파일을 갱신한다.
**상태:** 착수 전(PENDING) — 이 문서는 구현 계약이며 구현 완료를 의미하지 않는다.

## Context

추천 여행지 4 + CTA Banner을(를) 담당하는 COMPONENT Task다. `docs/06_SRS_UIUX_REVISED.md`와
...
- Implementation Status: **IMPLEMENT**(`docs/PROJECT_SCOPE.md` 분류를 그대로 승계 — `TASKS/00_TASK_LIST.md` Seq 18 행과 동일)
- 관련 Requirement: REQ-FUNC-062, 063
...
- 생성: `src/components/scr002/RecommendCta.tsx`

> 이 Task는 위에 나열된 파일만 생성/수정한다. 범위 밖 파일 수정은 "Forbidden" 절을 따른다.

## Functional AC

- 4개 Card(별점 배지 금지)+CTA Banner("여행 조건 정리"→`/travel-tools`, "동행 찾기"→`/mates`)

## Visual AC

- 카드 클릭 시 SCR-001 상세 Drawer로 이동
...
- [ ] TC-1: 4개 Card(별점 배지 금지)+CTA Banner("여행 조건 정리"→`/travel-tools`, "동행 찾기"→`/mates`)
