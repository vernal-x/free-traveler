...
**Seq:** 57 · **Category:** UNIT_TEST · **Priority:** M
**출처:** `TASKS/00_TASK_LIST.md` Seq 57 (Table A/B) — 이 파일이 상세화 대상이며, 두 문서가
어긋나면 `TASKS/00_TASK_LIST.md`를 정본으로 하고 이 파일을 갱신한다.
**상태:** 착수 전(PENDING) — 이 문서는 구현 계약이며 구현 완료를 의미하지 않는다.

## Context

연락처 탐지 단위 테스트을(를) 담당하는 UNIT_TEST Task다. `docs/06_SRS_UIUX_REVISED.md`와
...
- Implementation Status: **IMPLEMENT**(`docs/PROJECT_SCOPE.md` 분류를 그대로 승계 — `TASKS/00_TASK_LIST.md` Seq 57 행과 동일)
- 관련 Requirement: REQ-FUNC-032
...
- 생성: `src/lib/validation/__tests__/contact-detection.test.ts`

> 이 Task는 위에 나열된 파일만 생성/수정한다. 범위 밖 파일 수정은 "Forbidden" 절을 따른다.

## Functional AC

- 전화번호/이메일/카카오톡·텔레그램 ID 패턴 기준 테스트셋으로 탐지율 확인(정량 목표 미실시, 케이스 기반 확인으로 축소)
...
- [ ] TC-1: 전화번호/이메일/카카오톡·텔레그램 ID 패턴 기준 테스트셋으로 탐지율 확인(정량 목표 미실시, 케이스 기반 확인으로 축소)
