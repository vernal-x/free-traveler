...
**Seq:** 35 · **Category:** DATA · **Priority:** M
**출처:** `TASKS/00_TASK_LIST.md` Seq 35 (Table A/B) — 이 파일이 상세화 대상이며, 두 문서가
어긋나면 `TASKS/00_TASK_LIST.md`를 정본으로 하고 이 파일을 갱신한다.
**상태:** 착수 전(PENDING) — 이 문서는 구현 계약이며 구현 완료를 의미하지 않는다.

## Context

국가 안전정보 정적 데이터을(를) 담당하는 DATA Task다. `docs/06_SRS_UIUX_REVISED.md`와
...
- Implementation Status: **IMPLEMENT**(`docs/PROJECT_SCOPE.md` 분류를 그대로 승계 — `TASKS/00_TASK_LIST.md` Seq 35 행과 동일)
- 관련 Requirement: REQ-FUNC-046~054
...
- 생성: `src/data/country-safety.ts`, `src/data/country-safety.schema.ts`

> 이 Task는 위에 나열된 파일만 생성/수정한다. 범위 밖 파일 수정은 "Forbidden" 절을 따른다.

## Functional AC

- **정적 TypeScript 모듈, DB 테이블 아님.** 해외 국가 전체 커버리지, 8개 카테고리, 출처·확인일·경보범위 필수
...
- [ ] TC-1: **정적 TypeScript 모듈, DB 테이블 아님.** 해외 국가 전체 커버리지, 8개 카테고리, 출처·확인일·경보범위 필수
