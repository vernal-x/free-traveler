# DB-ACCESS — 타입 안전 Supabase 접근 레이어

**Seq:** 39 · **Category:** DB · **Priority:** M
**출처:** `TASKS/00_TASK_LIST.md` Seq 39 (Table A/B) — 이 파일이 상세화 대상이며, 두 문서가
어긋나면 `TASKS/00_TASK_LIST.md`를 정본으로 하고 이 파일을 갱신한다.
**상태:** 착수 전(PENDING) — 이 문서는 구현 계약이며 구현 완료를 의미하지 않는다.

## Context

타입 안전 Supabase 접근 레이어을(를) 담당하는 DB Task다. `docs/06_SRS_UIUX_REVISED.md`와
...
- Implementation Status: **IMPLEMENT**(`docs/PROJECT_SCOPE.md` 분류를 그대로 승계 — `TASKS/00_TASK_LIST.md` Seq 39 행과 동일)
- 관련 Requirement: REQ-NF-015, 017
...
- 생성: `src/lib/supabase/client.ts`, `server.ts`, `queries/*.ts`

> 이 Task는 위에 나열된 파일만 생성/수정한다. 범위 밖 파일 수정은 "Forbidden" 절을 따른다.

## Functional AC

- 6개 테이블에 대한 타입 안전 CRUD 헬퍼만 제공(다른 테이블 접근 함수 추가 금지)
...
- 입력 검증(zod) 후 쿼리, XSS 이스케이프

## Test Cases

- [ ] TC-1: 6개 테이블에 대한 타입 안전 CRUD 헬퍼만 제공(다른 테이블 접근 함수 추가 금지)
- [ ] TC-2(보안/개인정보): 입력 검증(zod) 후 쿼리, XSS 이스케이프
