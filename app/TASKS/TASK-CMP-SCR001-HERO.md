# CMP-SCR001-HERO — 검색 Hero(검색창+CTA)

**Seq:** 6 · **Category:** COMPONENT · **Priority:** M
**출처:** `TASKS/00_TASK_LIST.md` Seq 6 (Table A/B) — 이 파일이 상세화 대상이며, 두 문서가
어긋나면 `TASKS/00_TASK_LIST.md`를 정본으로 하고 이 파일을 갱신한다.
**상태:** 착수 전(PENDING) — 이 문서는 구현 계약이며 구현 완료를 의미하지 않는다.

## Context

검색 Hero(검색창+CTA)을(를) 담당하는 COMPONENT Task다. `docs/06_SRS_UIUX_REVISED.md`와
...
- Implementation Status: **IMPLEMENT**(`docs/PROJECT_SCOPE.md` 분류를 그대로 승계 — `TASKS/00_TASK_LIST.md` Seq 6 행과 동일)
- 관련 Requirement: REQ-FUNC-003, 064
...
- 생성: `src/components/scr001/HeroSearch.tsx`

> 이 Task는 위에 나열된 파일만 생성/수정한다. 범위 밖 파일 수정은 "Forbidden" 절을 따른다.

## Functional AC

- 키워드 검색창(도시·국가·테마 자동완성), "여행 준비 시작하기" 버튼(→`/travel-tools`)

## Visual AC

- `radius.pill` 검색바, 코랄 CTA 1개
...
- [ ] TC-1: 키워드 검색창(도시·국가·테마 자동완성), "여행 준비 시작하기" 버튼(→`/travel-tools`)
