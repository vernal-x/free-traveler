# CMP-SCR001-SAFETY-GRID-DRAWER — 국가별 주의사항 Card 6 + 안전정보 Drawer

**Seq:** 10 · **Category:** COMPONENT · **Priority:** M
**출처:** `TASKS/00_TASK_LIST.md` Seq 10 (Table A/B) — 이 파일이 상세화 대상이며, 두 문서가
어긋나면 `TASKS/00_TASK_LIST.md`를 정본으로 하고 이 파일을 갱신한다.
**상태:** 착수 전(PENDING) — 이 문서는 구현 계약이며 구현 완료를 의미하지 않는다.

## Context

국가별 주의사항 Card 6 + 안전정보 Drawer을(를) 담당하는 COMPONENT Task다. `docs/06_SRS_UIUX_REVISED.md`와
...
- Implementation Status: **IMPLEMENT**(`docs/PROJECT_SCOPE.md` 분류를 그대로 승계 — `TASKS/00_TASK_LIST.md` Seq 10 행과 동일)
- 관련 Requirement: REQ-FUNC-006, 047~054
- 각 Requirement의 축소/제외 사유·확인 방법 원문은 `docs/PROJECT_SCOPE.md`의 해당 Requirement 행을 정본으로 한다(이 파일에서 재서술하지 않음).

## Requirement Ref

REQ-FUNC-006, 047~054
...
- `design-reference/D-001/DESIGN.md` 컴포넌트 정의: Destination Card, Drawer·Modal
...
- 생성: `src/components/scr001/SafetyGrid.tsx`, `SafetyDrawer.tsx`

> 이 Task는 위에 나열된 파일만 생성/수정한다. 범위 밖 파일 수정은 "Forbidden" 절을 따른다.

## Functional AC

- 8개 카테고리, 경보 범위(국가/지역 구분), stale(7일 초과) 배지 렌더링 시 계산, 외교부 원문 링크(`noopener,noreferrer`), "공식 판단 대체 아님" 고지

## Visual AC

- 중대 경보는 텍스트 라벨과 함께 상단 배치(색상만 사용 금지)
...
- [ ] TC-1: 8개 카테고리, 경보 범위(국가/지역 구분), stale(7일 초과) 배지 렌더링 시 계산, 외교부 원문 링크(`noopener,noreferrer`), "공식 판단 대체 아님" 고지
