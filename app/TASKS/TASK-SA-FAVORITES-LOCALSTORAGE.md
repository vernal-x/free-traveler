# SA-FAVORITES-LOCALSTORAGE — 즐겨찾기(localStorage)

**Seq:** 48 · **Category:** SERVER_ACTION · **Priority:** S
**출처:** `TASKS/00_TASK_LIST.md` Seq 48 (Table A/B) — 이 파일이 상세화 대상이며, 두 문서가
어긋나면 `TASKS/00_TASK_LIST.md`를 정본으로 하고 이 파일을 갱신한다.
**상태:** 착수 전(PENDING) — 이 문서는 구현 계약이며 구현 완료를 의미하지 않는다.

## Context

즐겨찾기(localStorage)을(를) 담당하는 SERVER_ACTION Task다. `docs/06_SRS_UIUX_REVISED.md`와
...
- Implementation Status: **IMPLEMENT**(`docs/PROJECT_SCOPE.md` 분류를 그대로 승계 — `TASKS/00_TASK_LIST.md` Seq 48 행과 동일)
- 관련 Requirement: REQ-FUNC-068
...
- 생성: `src/lib/favorites.ts`

> 이 Task는 위에 나열된 파일만 생성/수정한다. 범위 밖 파일 수정은 "Forbidden" 절을 따른다.

## Functional AC

- **localStorage만 사용**, 서버 저장/동기화 없음, 중복 즐겨찾기 방지
...
- [ ] TC-1: **localStorage만 사용**, 서버 저장/동기화 없음, 중복 즐겨찾기 방지
- [ ] TC-2(보안/개인정보): 서버로 전송하지 않음
