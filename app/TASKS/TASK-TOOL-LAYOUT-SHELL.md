# TOOL-LAYOUT-SHELL — 전역 Header/Footer/layout.tsx

**Seq:** 51 · **Category:** TOOLING · **Priority:** M
**출처:** `TASKS/00_TASK_LIST.md` Seq 51 (Table A/B) — 이 파일이 상세화 대상이며, 두 문서가
어긋나면 `TASKS/00_TASK_LIST.md`를 정본으로 하고 이 파일을 갱신한다.
**상태:** 착수 전(PENDING) — 이 문서는 구현 계약이며 구현 완료를 의미하지 않는다.

## Context

전역 Header/Footer/layout.tsx을(를) 담당하는 TOOLING Task다. `docs/06_SRS_UIUX_REVISED.md`와
...
- Implementation Status: **IMPLEMENT(축소)**(`docs/PROJECT_SCOPE.md` 분류를 그대로 승계 — `TASKS/00_TASK_LIST.md` Seq 51 행과 동일)
- 관련 Requirement: REQ-FUNC-027, 043, 064, 065, 079
...
- `design-reference/D-001/DESIGN.md` § Header·Footer, § Color/Typography/Spacing 토큰

## Depends On

- TOOL-DESIGN-TOKENS
- SA-TOAST-NOTIFICATIONS

## Expected Files

- 수정: `src/app/layout.tsx`(현재 Next.js 기본 스캐폴드 — 교체)

> 이 Task는 위에 나열된 파일만 생성/수정한다. 범위 밖 파일 수정은 "Forbidden" 절을 따른다.

## Functional AC

- Header(로고/nav 4개/로그인·계정)·Footer(4컬럼) 5개 화면 공통, 미인증 쓰기 시도 시 SCR-005로 리다이렉트, Toast 컴포넌트 마운트

## Visual AC

- Desktop 72px/Mobile 56px Header, 스크롤 8px↑에서만 `shadow.card`
...
- [ ] TC-1: Header(로고/nav 4개/로그인·계정)·Footer(4컬럼) 5개 화면 공통, 미인증 쓰기 시도 시 SCR-005로 리다이렉트, Toast 컴포넌트 마운트
