# CMP-SCR003-DISCLOSURE-TIPS — 비전달 고지 + Tip 3개

**Seq:** 22 · **Category:** COMPONENT · **Priority:** S
**출처:** `TASKS/00_TASK_LIST.md` Seq 22 (Table A/B) — 이 파일이 상세화 대상이며, 두 문서가
어긋나면 `TASKS/00_TASK_LIST.md`를 정본으로 하고 이 파일을 갱신한다.
**상태:** 착수 전(PENDING) — 이 문서는 구현 계약이며 구현 완료를 의미하지 않는다.

## Context

비전달 고지 + Tip 3개을(를) 담당하는 COMPONENT Task다. `docs/06_SRS_UIUX_REVISED.md`와
...
- Implementation Status: **IMPLEMENT**(`docs/PROJECT_SCOPE.md` 분류를 그대로 승계 — `TASKS/00_TASK_LIST.md` Seq 22 행과 동일)
- 관련 Requirement: REQ-FUNC-015, 017, 023, 025, 054
- 각 Requirement의 축소/제외 사유·확인 방법 원문은 `docs/PROJECT_SCOPE.md`의 해당 Requirement 행을 정본으로 한다(이 파일에서 재서술하지 않음).

## Requirement Ref

REQ-FUNC-015, 017, 023, 025, 054
...
- 생성: `src/components/scr003/DisclosureTips.tsx`

> 이 Task는 위에 나열된 파일만 생성/수정한다. 범위 밖 파일 수정은 "Forbidden" 절을 따른다.

## Functional AC

- 고지 문구 상시 노출, 팁 카드 3개(항공권 검색 시기/숙소 교통 접근성/안전정보 재확인, 안전정보 링크는 SCR-001 Drawer로 연결)
...
- [ ] TC-1: 고지 문구 상시 노출, 팁 카드 3개(항공권 검색 시기/숙소 교통 접근성/안전정보 재확인, 안전정보 링크는 SCR-001 Drawer로 연결)
