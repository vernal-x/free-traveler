# AUTH-EMAIL-ADULT — Supabase 이메일 인증 + 성인 확인

**Seq:** 41 · **Category:** AUTH · **Priority:** M
**출처:** `TASKS/00_TASK_LIST.md` Seq 41 (Table A/B) — 이 파일이 상세화 대상이며, 두 문서가
어긋나면 `TASKS/00_TASK_LIST.md`를 정본으로 하고 이 파일을 갱신한다.
**상태:** 착수 전(PENDING) — 이 문서는 구현 계약이며 구현 완료를 의미하지 않는다.

## Context

Supabase 이메일 인증 + 성인 확인을(를) 담당하는 AUTH Task다. `docs/06_SRS_UIUX_REVISED.md`와
...
- Implementation Status: **IMPLEMENT**(`docs/PROJECT_SCOPE.md` 분류를 그대로 승계 — `TASKS/00_TASK_LIST.md` Seq 41 행과 동일)
- 관련 Requirement: REQ-FUNC-027, 028, 066
...
- 생성: `src/lib/auth/*.ts`, `src/app/auth/callback/route.ts`(기술 Route)

> 이 Task는 위에 나열된 파일만 생성/수정한다. 범위 밖 파일 수정은 "Forbidden" 절을 따른다.

## Functional AC

- 이메일 가입/로그인/로그아웃/재설정, 성인확인 단계(생년월일 미저장)
...
- [ ] TC-1: 이메일 가입/로그인/로그아웃/재설정, 성인확인 단계(생년월일 미저장)
- [ ] TC-2(보안/개인정보): 미인증 쓰기 요청 401/리다이렉트
