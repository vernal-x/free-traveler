# CMP-SCR005-PROFILE — 프로필 편집 + 탈퇴

**Seq:** 30 · **Category:** COMPONENT · **Priority:** M
**출처:** `TASKS/00_TASK_LIST.md` Seq 30 (Table A/B) — 이 파일이 상세화 대상이며, 두 문서가
어긋나면 `TASKS/00_TASK_LIST.md`를 정본으로 하고 이 파일을 갱신한다.
**상태:** 착수 전(PENDING) — 이 문서는 구현 계약이며 구현 완료를 의미하지 않는다.

## Context

프로필 편집 + 탈퇴을(를) 담당하는 COMPONENT Task다. `docs/06_SRS_UIUX_REVISED.md`와
...
- Implementation Status: **IMPLEMENT(축소)**(`docs/PROJECT_SCOPE.md` 분류를 그대로 승계 — `TASKS/00_TASK_LIST.md` Seq 30 행과 동일)
- 관련 Requirement: REQ-FUNC-029, 045
...
- 생성: `src/components/scr005/ProfileForm.tsx`

> 이 Task는 위에 나열된 파일만 생성/수정한다. 범위 밖 파일 수정은 "Forbidden" 절을 따른다.

## Functional AC

- 닉네임/연령대/성별(선택)/스타일/자기소개 편집, 탈퇴 버튼(즉시 비식별화, 30일 유예·법적 보존 예외는 이번 범위 제외)
...
- 탈퇴 즉시 처리, 신분증 업로드 UI 없음

## Test Cases

- [ ] TC-1: 닉네임/연령대/성별(선택)/스타일/자기소개 편집, 탈퇴 버튼(즉시 비식별화, 30일 유예·법적 보존 예외는 이번 범위 제외)
- [ ] TC-2(보안/개인정보): 탈퇴 즉시 처리, 신분증 업로드 UI 없음
