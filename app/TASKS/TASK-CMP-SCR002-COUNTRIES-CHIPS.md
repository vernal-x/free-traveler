# CMP-SCR002-COUNTRIES-CHIPS — 방문 국가 Chip 30개

**Seq:** 16 · **Category:** COMPONENT · **Priority:** S
**출처:** `TASKS/00_TASK_LIST.md` Seq 16 (Table A/B) — 이 파일이 상세화 대상이며, 두 문서가
어긋나면 `TASKS/00_TASK_LIST.md`를 정본으로 하고 이 파일을 갱신한다.
**상태:** 착수 전(PENDING) — 이 문서는 구현 계약이며 구현 완료를 의미하지 않는다.

## Context

방문 국가 Chip 30개을(를) 담당하는 COMPONENT Task다. `docs/06_SRS_UIUX_REVISED.md`와
...
- Implementation Status: **IMPLEMENT**(`docs/PROJECT_SCOPE.md` 분류를 그대로 승계 — `TASKS/00_TASK_LIST.md` Seq 16 행과 동일)
- 관련 Requirement: REQ-FUNC-059
...
- 생성: `src/components/scr002/CountryChips.tsx`

> 이 Task는 위에 나열된 파일만 생성/수정한다. 범위 밖 파일 수정은 "Forbidden" 절을 따른다.

## Functional AC

- 정확히 30개국(2026-09-15 Stitch 검증에서 28→30 보정된 값), 권역 4그룹(아시아/유럽/북미/오세아니아)

## Visual AC

- Desktop wrap, Mobile 가로 스크롤
...
- [ ] TC-1: 정확히 30개국(2026-09-15 Stitch 검증에서 28→30 보정된 값), 권역 4그룹(아시아/유럽/북미/오세아니아)
