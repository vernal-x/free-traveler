# CMP-SCR005-ADMIN-REPORTS — 신고 상태 관리(축소)

**Seq:** 32 · **Category:** COMPONENT · **Priority:** M
**출처:** `TASKS/00_TASK_LIST.md` Seq 32 (Table A/B) — 이 파일이 상세화 대상이며, 두 문서가
어긋나면 `TASKS/00_TASK_LIST.md`를 정본으로 하고 이 파일을 갱신한다.
**상태:** 착수 전(PENDING) — 이 문서는 구현 계약이며 구현 완료를 의미하지 않는다.

## Context

신고 상태 관리(축소)을(를) 담당하는 COMPONENT Task다. `docs/06_SRS_UIUX_REVISED.md`와
...
- Implementation Status: **IMPLEMENT(축소)**(`docs/PROJECT_SCOPE.md` 분류를 그대로 승계 — `TASKS/00_TASK_LIST.md` Seq 32 행과 동일)
- 관련 Requirement: REQ-FUNC-041
...
- 생성: `src/components/scr005/AdminReports.tsx`

> 이 Task는 위에 나열된 파일만 생성/수정한다. 범위 밖 파일 수정은 "Forbidden" 절을 따른다.

## Functional AC

- 상태 필터(OPEN/RESOLVED/DISMISSED)만 있는 목록 + 상태 변경, 경고/계정정지 등 개별 제재 UI는 만들지 않음(REQ-FUNC-042 EXCLUDED 준수)
...
- Admin/Moderator 역할만 접근(RLS)

## Test Cases

- [ ] TC-1: 상태 필터(OPEN/RESOLVED/DISMISSED)만 있는 목록 + 상태 변경, 경고/계정정지 등 개별 제재 UI는 만들지 않음(REQ-FUNC-042 EXCLUDED 준수)
- [ ] TC-2(보안/개인정보): Admin/Moderator 역할만 접근(RLS)
