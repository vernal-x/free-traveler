# TOOL-POLICY-PAGES — 정적 정책 페이지(약관/개인정보/안전수칙/면책)

**Seq:** 55 · **Category:** TOOLING · **Priority:** M
**출처:** `TASKS/00_TASK_LIST.md` Seq 55 (Table A/B) — 이 파일이 상세화 대상이며, 두 문서가
어긋나면 `TASKS/00_TASK_LIST.md`를 정본으로 하고 이 파일을 갱신한다.
**상태:** 착수 전(PENDING) — 이 문서는 구현 계약이며 구현 완료를 의미하지 않는다.

## Context

정적 정책 페이지(약관/개인정보/안전수칙/면책)을(를) 담당하는 TOOLING Task다. `docs/06_SRS_UIUX_REVISED.md`와
...
- Implementation Status: **IMPLEMENT**(`docs/PROJECT_SCOPE.md` 분류를 그대로 승계 — `TASKS/00_TASK_LIST.md` Seq 55 행과 동일)
- 관련 Requirement: REQ-FUNC-080
...
- Screen: 비핵심 정적 페이지
- Route: `/legal/*`
- Page Entry: `src/app/legal/*/page.tsx`
...
- 생성: `src/app/legal/terms/page.tsx`, `privacy/page.tsx`, `mate-safety/page.tsx`, `content-disclaimer/page.tsx`

> 이 Task는 위에 나열된 파일만 생성/수정한다. 범위 밖 파일 수정은 "Forbidden" 절을 따른다.

## Functional AC

- 이용약관/개인정보처리방침/동행 안전수칙/콘텐츠 이용 안내 4종, 동행 작성 시 안전수칙 동의 시각 저장과 연동

## Visual AC

- Footer에서 링크, 5개 핵심 Screen 수에 미포함
...
- [ ] TC-1: 이용약관/개인정보처리방침/동행 안전수칙/콘텐츠 이용 안내 4종, 동행 작성 시 안전수칙 동의 시각 저장과 연동
