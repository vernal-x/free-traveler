# CMP-SCR003-INTRO-TABS — Intro + 탭 셸(항공/숙소/동행)

**Seq:** 19 · **Category:** COMPONENT · **Priority:** M
**출처:** `TASKS/00_TASK_LIST.md` Seq 19 (Table A/B) — 이 파일이 상세화 대상이며, 두 문서가
어긋나면 `TASKS/00_TASK_LIST.md`를 정본으로 하고 이 파일을 갱신한다.
**상태:** 착수 전(PENDING) — 이 문서는 구현 계약이며 구현 완료를 의미하지 않는다.

## Context

Intro + 탭 셸(항공/숙소/동행)을(를) 담당하는 COMPONENT Task다. `docs/06_SRS_UIUX_REVISED.md`와
...
- Implementation Status: **IMPLEMENT**(`docs/PROJECT_SCOPE.md` 분류를 그대로 승계 — `TASKS/00_TASK_LIST.md` Seq 19 행과 동일)
- 관련 Requirement: REQ-FUNC-064
...
- 생성: `src/components/scr003/TravelToolsIntro.tsx`, `Tabs.tsx`

> 이 Task는 위에 나열된 파일만 생성/수정한다. 범위 밖 파일 수정은 "Forbidden" 절을 따른다.

## Functional AC

- 3단계 요약 문구, 탭 3개(항공편/숙소/동행 구하기), 탭 전환 시 다른 탭 입력값 유지

## Visual AC

- 활성 탭 코랄 밑줄, Mobile 세그먼트 컨트롤
...
- [ ] TC-1: 3단계 요약 문구, 탭 3개(항공편/숙소/동행 구하기), 탭 전환 시 다른 탭 입력값 유지
