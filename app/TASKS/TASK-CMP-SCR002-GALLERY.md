# CMP-SCR002-GALLERY — 여행 사진 Gallery 8장↑

**Seq:** 17 · **Category:** COMPONENT · **Priority:** S
**출처:** `TASKS/00_TASK_LIST.md` Seq 17 (Table A/B) — 이 파일이 상세화 대상이며, 두 문서가
어긋나면 `TASKS/00_TASK_LIST.md`를 정본으로 하고 이 파일을 갱신한다.
**상태:** 착수 전(PENDING) — 이 문서는 구현 계약이며 구현 완료를 의미하지 않는다.

## Context

여행 사진 Gallery 8장↑을(를) 담당하는 COMPONENT Task다. `docs/06_SRS_UIUX_REVISED.md`와
...
- Implementation Status: **IMPLEMENT(축소)**(`docs/PROJECT_SCOPE.md` 분류를 그대로 승계 — `TASKS/00_TASK_LIST.md` Seq 17 행과 동일)
- 관련 Requirement: REQ-FUNC-061
...
- 생성: `src/components/scr002/Gallery.tsx`

> 이 Task는 위에 나열된 파일만 생성/수정한다. 범위 밖 파일 수정은 "Forbidden" 절을 따른다.

## Functional AC

- 서로 다른 장소 8장↑, 각 alt에 실제 장소 서술, **별점/평점 배지 금지**(재발 방지 — 2026-09-15 발견 항목)
...
- 이미지 alt 필수, 출처/작가는 축소 캡션만

## Test Cases

- [ ] TC-1: 서로 다른 장소 8장↑, 각 alt에 실제 장소 서술, **별점/평점 배지 금지**(재발 방지 — 2026-09-15 발견 항목)
- [ ] TC-2(보안/개인정보): 이미지 alt 필수, 출처/작가는 축소 캡션만
