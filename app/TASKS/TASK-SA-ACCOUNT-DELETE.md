# SA-ACCOUNT-DELETE — 탈퇴 즉시 비식별화

**Seq:** 47 · **Category:** SERVER_ACTION · **Priority:** M
**출처:** `TASKS/00_TASK_LIST.md` Seq 47 (Table A/B) — 이 파일이 상세화 대상이며, 두 문서가
어긋나면 `TASKS/00_TASK_LIST.md`를 정본으로 하고 이 파일을 갱신한다.
**상태:** 착수 전(PENDING) — 이 문서는 구현 계약이며 구현 완료를 의미하지 않는다.

## Context

탈퇴 즉시 비식별화을(를) 담당하는 SERVER_ACTION Task다. `docs/06_SRS_UIUX_REVISED.md`와
...
- Implementation Status: **IMPLEMENT(축소)**(`docs/PROJECT_SCOPE.md` 분류를 그대로 승계 — `TASKS/00_TASK_LIST.md` Seq 47 행과 동일)
- 관련 Requirement: REQ-FUNC-045
...
- 생성: `src/lib/actions/account-delete.ts`

> 이 Task는 위에 나열된 파일만 생성/수정한다. 범위 밖 파일 수정은 "Forbidden" 절을 따른다.

## Functional AC

- 즉시 비식별화 + 비필수 개인정보 삭제(30일 유예 배치 없음, 축소 범위 명시)
...
- 처리 결과를 `report`/`user_block` 등 관련 레코드에 orphan 남기지 않음

## Test Cases

- [ ] TC-1: 즉시 비식별화 + 비필수 개인정보 삭제(30일 유예 배치 없음, 축소 범위 명시)
- [ ] TC-2(보안/개인정보): 처리 결과를 `report`/`user_block` 등 관련 레코드에 orphan 남기지 않음
