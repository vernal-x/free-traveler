...
**Seq:** 8 · **Category:** COMPONENT · **Priority:** M
**출처:** `TASKS/00_TASK_LIST.md` Seq 8 (Table A/B) — 이 파일이 상세화 대상이며, 두 문서가
어긋나면 `TASKS/00_TASK_LIST.md`를 정본으로 하고 이 파일을 갱신한다.
**상태:** 착수 전(PENDING) — 이 문서는 구현 계약이며 구현 완료를 의미하지 않는다.

## Context

여행지 상세 Drawer/Modal을(를) 담당하는 COMPONENT Task다. `docs/06_SRS_UIUX_REVISED.md`와
...
- Implementation Status: **IMPLEMENT**(`docs/PROJECT_SCOPE.md` 분류를 그대로 승계 — `TASKS/00_TASK_LIST.md` Seq 8 행과 동일)
- 관련 Requirement: REQ-FUNC-004, 006, 007, 009
...
- 생성: `src/components/scr001/DestinationDrawer.tsx`

> 이 Task는 위에 나열된 파일만 생성/수정한다. 범위 밖 파일 수정은 "Forbidden" 절을 따른다.

## Functional AC

- 소개 300자↑, 명소 5개↑, 1일/3일 일정, 예산, 교통, 음식 3개↑, 에티켓 3개↑, 출처·수정일, 관련 여행지 최대 6개, "안전정보 보기" 전환

## Visual AC

- Desktop 우측 480px Drawer, Mobile 전체화면 Modal
...
- [ ] TC-1: 소개 300자↑, 명소 5개↑, 1일/3일 일정, 예산, 교통, 음식 3개↑, 에티켓 3개↑, 출처·수정일, 관련 여행지 최대 6개, "안전정보 보기" 전환
